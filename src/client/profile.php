...
$client_sql = "SELECT users.*
        FROM users
        WHERE users.`id` = $user_id
        ;";
$client = mysqli_fetch_assoc(mysqli_query($conn, $client_sql));
...
$projects_sql = "SELECT  p.`id`
                ...
                ;";
$user_projects_res = mysqli_query($conn, $projects_sql);
$posted_projects_sql = "SELECT COUNT(*) as posted_projects FROM projects p WHERE p.user_id = $user_id"; 
$posted_projects = mysqli_fetch_assoc(mysqli_query($conn, $posted_projects_sql))['posted_projects'];
                    ...
                    <div class="px-6">
                        ...
                        <div class="flex flex-wrap justify-center">
                            <div class="w-full lg:w-3/12 px-4 lg:order-2 flex justify-center">
                                <div class="">
                                    <img alt="your photo" src="<?= $path . $client['photo_src'] ?>" class="shadow-xl rounded-full h-auto align-middle border-none -mt-20 max-w-150-px">
                                </div>
                            </div>
                            <?php
                            if (!empty($client)) {
                                ...
                                ?>
                                    <div class="w-full lg:w-4/12 px-4 lg:order-3 lg:text-right lg:self-center">
                                        <form class="py-6 px-3 mt-32 sm:mt-0" action="../delete.php" method="POST">
                                            ...
                                            <button class="bg-red-500 active:bg-pink-600 uppercase text-white font-bold hover:shadow-md shadow text-xs px-4 py-2 rounded outline-none focus:outline-none sm:mr-2 mb-1 ease-linear transition-all duration-150"
                                                    type="submit" onclick="return confirmDelete()">
                                                    Remove
                                            </button>
                                        </form>
                                    </div>
                            ...
                            <div class="w-full lg:w-4/12 px-4 lg:order-1">
                                <div class="flex justify-centerlg:pt-4 ">
                                    <div class="mr-4 p-3 text-center">
                                        <span class="text-xl font-bold block uppercase tracking-wide text-blueGray-600"><?= $client['status'] ?></span><span class="text-sm text-blueGray-400"><?= $posted_projects ?> posted projects</span>
                                    </div>
                                    <div class="mr-4 p-3 text-center">
                                        <span class="text-xl font-bold block tracking-wide text-blueGray-600"><?= $client['email'] ?></span><span class="text-sm text-blueGray-400">email</span>
                                    </div>
                                    <div class="lg:mr-4 p-3 text-center">
                                        <span class="text-xl font-bold block tracking-wide text-blueGray-600"><?= $client['rating'] ?></span><span class="text-sm text-blueGray-400">Rating</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="py-10 border-t border-blueGray-200 text-center">
                                <div class="flex flex-wrap justify-center">
                                    ...
                                    <table class="table-auto w-full text-sm whitespace-no-wrap border-spacing-2">
                                        ...
                                        <tbody>
                                            <?php
                                            if (mysqli_num_rows($user_projects_res) > 0) {
                                                while ($row = mysqli_fetch_assoc($user_projects_res)) :
                                                    $project_id = $row['id'];
                                                    $title = $row['title'];
                                                    $price = $row['price'];
                                            ...
                                            ?>
                                                    <tr class="odd:bg-gray-200 even:bg-gray-300">
                                                        <td class="p-1 text-center border-r border-white">
                                                            <a href="<?= "../project/view.php?id=" . $project_id ?>" class="p-1  rounded-full border border-sky-500 text-sky-500 text-xl hover:py-0 hover:text-2xl">
                                                                +
                                                            </a>
                                                        </td>
                                                            ...
                                                            <td class="p-1 h-full border-r border-white flex flex-col justify-center">
                                                                <div class="relative w-full h-full mx-auto">
                                                                    ...
                                                                    <?php
                                                                        ...
                                                                        $sql2 = "SELECT u.id as freelancer_id,
                                                                        ...
                                                                        if (mysqli_num_rows($freelancers) > 0) {
                                                                            ?>
                                                                            <button class="top-0 mb-1 assigne-btn p-2 text-white bg-blue-500 rounded-md hover:bg-blue-600">
                                                                                Assigne a freelancer
                                                                                <small class="p-1 rounded-full w-3 h-3 bg-yellow-400"><?= mysqli_num_rows($freelancers) ?></small>
                                                                            </button>
                                                                        <?php
                                                                        }else{
                                                                        ?>
                                                                            <p>No proposal sended.</p>
                                                                        <?php
                                                                        }
                                                                        ?>

                                                                    <div class="d-none flex flex-row gap-x-2">
                                                                        <form class="flex flex-row justify-around w-full items-center" action="../project/assigne.php" method="POST">
                                                                            <select class="freelancers-select top-9 z-50 left-0 float-left p-2 bg-gray-200 rounded-md" name="freelancer_id" id="freelancer">
                                                                                <option class="d-none" value="" selected disabled>Select</option>
                                                                                <?php
                                                                                    while ($freelancer = mysqli_fetch_assoc($freelancers)) :
                                                                                        $freelancer_id = $freelancer['freelancer_id'];
                                                                                        $freelancer_name = $freelancer['freelancer_username'];
                                                                                        $freelancer_photo_src = $freelancer['freelancer_photo_src'];
                                                                                ?>
                                                                            ...
                                                                            <button class="hover:bg-sky-900 bg-sky-300 text-sky-900 border rounded-md p-2 border-sky-900 hover:text-white" name="project_id" value="<?= $project_id ?>">
                                                                                Save
                                                                            </button>
                                                                        </form>
                                                                    </div>
